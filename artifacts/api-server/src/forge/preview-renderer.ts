...
const fontLinks: Record<WizardState["preset"], string> = {
  editorial: `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Instrument+Serif&family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400&display=swap" rel="stylesheet">`,
  minimalist: `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">`,
  aurora: `<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Inter:wght@200;400;500;700&family=JetBrains+Mono:wght@400&display=swap" rel="stylesheet">`,
};
...
function navLinks(state: WizardState, accentColor = false): string {
  const items: string[] = [];
...
<style>
  ...
  h1, h2, h3 { font-family: 'Instrument Serif', Georgia, serif; font-weight: 400; letter-spacing: -0.02em; line-height: 1.05; text-wrap: balance; margin: 0; }
  ...
  nav .name { font-family: 'Instrument Serif', serif; font-size: 1.25rem; }
  ...
  .loc { margin-top: 1rem; font-family: 'JetBrains Mono', monospace; font-size: 0.85rem; color: var(--color-mute); }
  .work { margin-top: 4rem; }
  .pcard { border-top: 1px solid var(--color-border); padding: 2rem 0; position: relative; transition: transform 200ms; }
  ...
  .pcard .num { font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; color: var(--color-mute); }
  .summary { color: var(--color-mute); margin: 0; }
  .chip { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.06em; padding: 0.2rem 0.5rem; border: 1px solid var(--color-border); margin: 0.5rem 0.4rem 0 0; color: var(--color-mute); }
...
<style>
  ${tokenCss(state)}
  body { font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 14px; padding: 4rem 2rem; max-width: 640px; }
  ...
  h1 { font-size: 1rem; font-weight: 700; margin: 0 0 1rem; }
...
<style>
  ${tokenCss(state)}
  body { font-family: 'Inter', system-ui, sans-serif; overflow-x: hidden; min-height: 100vh; }
  h1, h2, h3 { font-family: 'Inter', sans-serif; font-weight: 200; letter-spacing: -0.04em; line-height: 1; text-wrap: balance; margin: 0; }
  ...
  .eyebrow { font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.15em; color: var(--color-accent); margin: 0 0 1.5rem; }
  ...
  .loc { margin-top: 1.5rem; font-family: 'JetBrains Mono', monospace; font-size: 0.85rem; color: var(--color-mute); }
  .work { margin-top: 3rem; }
  .work h2 { font-size: 2.25rem; margin-bottom: 2rem; }
  .grid { display: grid; grid-template-columns: 1fr; gap: 1.25rem; }
  @media (min-width: 768px) { .grid { grid-template-columns: 1fr 1fr; } }
  .card { background: color-mix(in oklch, var(--color-surface) 60%, transparent); border: 1px solid var(--color-border); border-radius: 1.25rem; padding: 1.75rem; backdrop-filter: blur(12px); transition: transform 300ms, border-color 300ms; }
  .card:hover { transform: translateY(-4px); border-color: var(--color-accent); }
  .card .num { font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; color: var(--color-accent); }
  .card h3 { font-size: 1.5rem; margin: 0.5rem 0; font-weight: 300; }
  .card p { color: var(--color-mute); margin: 0; }
  .chip { display: inline-block; font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; padding: 0.2rem 0.6rem; background: color-mix(in oklch, var(--color-accent) 15%, transparent); color: var(--color-accent); border-radius: 999px; margin: 0.5rem 0.4rem 0 0; }
